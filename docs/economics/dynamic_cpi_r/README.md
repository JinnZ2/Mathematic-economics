...
| `legacy/docs/economics/dynamic_cpi_r/drafts/` | Retired (ledger `L007`) | The two non-runnable prototypes (`dynamic_inflation_weight.py`, `iteration_module.py`), moved out per this folder's own AUDIT.md recommendation to "preserve as an archival draft". |
...
| Research status | The draft files are retired to `legacy/` under ledger record `L007` and have not been promoted into the working module. | Provenance stays clear, and `L007`'s `precedence` field enumerates the five drafted mechanisms the working module never implemented — error tracking with bias EMA, adaptive gain tuning, external validation hooks, drift detection, and a revision log. They are unevaluated, not rejected. |
